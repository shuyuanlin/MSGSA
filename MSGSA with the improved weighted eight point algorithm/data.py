...
class CorrespondencesDataset(data.Dataset):
    ...
    def correctMatches(self, e_gt):
        step = 0.1
        xx,yy = np.meshgrid(np.arange(-1, 1, step), np.arange(-1, 1, step)) #生成20*20的网格
        # Points in first image before projection(
        # 投影前第一张图像中的点)第一幅图片的网格点坐标
        pts1_virt_b = np.float32(np.vstack((xx.flatten(), yy.flatten())).T) #400*2,网格点坐标，顺序右--->上
        # Points in second image before projection
        pts2_virt_b = np.float32(pts1_virt_b)   #第二幅图像网格点坐标
        pts1_virt_b, pts2_virt_b = pts1_virt_b.reshape(1,-1,2), pts2_virt_b.reshape(1,-1,2)

        pts1_virt_b, pts2_virt_b = cv2.correctMatches(e_gt.reshape(3,3), pts1_virt_b, pts2_virt_b) #cv2.correctMatches() # 修正对应点的坐标
